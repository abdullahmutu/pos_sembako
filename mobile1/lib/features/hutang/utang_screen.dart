...
import 'package:mobile1/features/hutang/detail_piutang_screen.dart';
  ...
  factory Piutang.fromJson(Map<String, dynamic> json) {
    ...
    return Piutang(
      ...
      nama: customer?['name'] ?? customer?['nama'] ??
            json['nama'] ?? json['name'] ?? '',
      imageUrl: customer?['image_url'] ?? customer?['photo'] ?? json['image_url'],
      jumlah: parseDouble(
        json['remaining'] ??
      ...
      terakhirTransaksi: DateTime.tryParse(
              json['created_at'] ?? json['last_transaction'] ??
              json['terakhir_transaksi'] ?? '') ?? DateTime.now(),
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        Text(_formatRupiah.format(totalDaftarHutang), // ← pakai ini
